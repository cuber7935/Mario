...
bool LayerModel::init()
{
	...
	/* 创建瓦片地图，瓦片地图的下标在LayerMapSelect场景中，已经选择过，并且保持在全局的Util::_mapIndex */
	initMap();	

	/* 初始化Roles，其实游戏角色属性都保存在瓦片地图的objects对象层，
	...
	*/
	...
	/* 增加自定义消息监听器 */
	this->initHitBlockEvent();
	this->initBossFireEvent();
	this->initBreakBridgeEvent();

	return true;
}

void LayerModel::initHitBlockEvent()
{
	EventListenerCustom* cus = EventListenerCustom::create("hitblock", CC_CALLBACK_1(LayerModel::hitBlock, this));
...
{
	EventListenerCustom* cus = EventListenerCustom::create("bossfire", CC_CALLBACK_1(LayerModel::bossFire, this));
	_eventDispatcher->addEventListenerWithSceneGraphPriority(cus, this);
}
...
// BOSS发射子弹 处理函数
void LayerModel::bossFire(EventCustom* ev)
{
	/* 执行发射子弹 */
...
// Mario 顶动砖头 处理函数
void LayerModel::hitBlock(EventCustom* ev)
{
	// 获取自定义消息参数：是一个精灵
...
// 获取当前地图
TMXTiledMap* LayerModel::getCurTiledMap()const
{
	return this->_map;
}